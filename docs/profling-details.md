...
- 浏览器的环境可以使用playwright做自动化。这个交给ai自动生成好了。目前看起来会输出 summary.json 和 .cpuprofile 两个文件。（现在的大模型完全可以自主地生成脚本来过滤 .cpuprofile文件，之前还考虑为这种格式地文件创建一个mcp server，完全没必要
- 测试数据可以选为`im2latex_formulas.txt`这个大文件。
- 设计好playwright脚本和之前用到的harnnes 测试流程后，ai会自动做迭代。
...
js.mean:   0.8611 ms
js.p95:    2.0000 ms
mean ratio (JS/WASM): 0.5261x
p95 ratio  (JS/WASM): 0.5556x
```
