...
fn delim_from_value(delim_string: &str) -> Option<String> {
    if delim_string.is_empty() || delim_string == "." {
        None
    } else {
        Some(delim_string.to_string())
    }
}

fn style_from_arg(arg: &Box<dyn AnyParseNode>) -> String {
    let style_index = if let Some(ord_group) = arg.as_any().downcast_ref::<parse_node::types::ordgroup>() {
        ord_group
            .body
            .first()
            .and_then(|node| {
                node.as_any()
                    ...
                    .and_then(|text_ord| text_ord.text.parse::<usize>().ok())
            })
    } else {
        arg.as_any()
            .downcast_ref::<parse_node::types::textord>()
...
pub fn genfrac_handler_fn(
    ...
    let context = ctx.borrow();
    let left_delim = normalize_argument(&args[0])
        .as_any()
    ...
    let right_delim = normalize_argument(&args[1])
        .as_any()
        .downcast_ref::<parse_node::types::atom>()
        .and_then(|atom| {
    ...
    let bar_node = args[2]
        ...
        .unwrap();
    let (has_bar_line, bar_size) = if bar_node.isBlank {
        (true, None)
    } else {
...
lazy_static! {
    pub static ref GENFRAC : Mutex<FunctionDefSpec> = Mutex::new({
        let mut props = FunctionPropSpec::new();
        props.set_num_args(6);
        props.set_allowed_in_argument(true);
        props.set_arg_types(vec![
            ArgType::math,
            ArgType::math,
            ArgType::size,
            ArgType::text,
...
lazy_static! {
    pub static ref INFIX: Mutex<FunctionDefSpec> = Mutex::new({
        ...
        FunctionDefSpec{
            ...
            props,
            handler: above_infix_handler_fn,
            html_builder: None,
            mathml_builder: None,
        }
    });
...
pub fn above_frac_handler_fn(
    ...
    let infix = args[1]
        ...
        .unwrap();
    let bar_size = infix.size.clone().unwrap();
    let res = parse_node::types::genfrac {
        mode: context.parser.mode,
        loc: None,
        continued: false,
        numer: args[0].clone(),
        denom: args[2].clone(),
        hasBarLine: bar_size.number > 0.0,
        leftDelim: None,
        rightDelim: None,
        size: "auto".to_string(),
        barSize: Some(bar_size),
    };
    Box::new(res) as Box<dyn AnyParseNode>
}
lazy_static! {
    pub static ref ABOVEFRAC : Mutex<FunctionDefSpec> = Mutex::new({
        let mut props = FunctionPropSpec::new();
        props.set_num_args(3);
        props.set_arg_types(vec![ArgType::math, ArgType::size, ArgType::math]);
